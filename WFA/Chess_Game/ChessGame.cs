...
namespace PA6_Draft
{
    ...
    internal delegate void WarningSound();
    internal delegate void InvalidateBoard();
    ...
    class ChessGame : INotifyPropertyChanged
    {
        ...
        internal event ChessEvent Started;
        internal event ChessEvent Moved;
        internal event ChessEvent Castled;
        internal event ChessEvent CheckmateEvent;
        internal event ChessEvent StalemateEvent;
        internal event ChessEvent CapturedPieceEvent;
        internal event ChessEvent CheckEvent;
        internal event WarningSound TenSeconds;
        internal event InvalidateBoard Invalidateboard;
        internal bool isCapture = false;
        internal bool PlayedWarning = false;
        ...
        public string WhiteTimeLimit
        {
            ...
            set
            {
                ...
                this.InvokePropertyChanged("WhiteTimeLimit");
            }
        }
        ...
        public string BlackTimeLimit
        {
            ...
            set
            {
                ...
                this.InvokePropertyChanged("BlackTimeLimit");
            }
        }
        ...
        internal bool Move(Move move)//If the move is not legal, returns false, otherwise, it makes the move and returns true...
        {
            ...
            if (move.Stalemate == true)
            {
                StalemateEvent(move);
            }
            move.Check = IsCheck(!WhiteTurn) && !move.Checkmate;
            if (move.Check == true && move.Checkmate == false && move.Stalemate == false)
            {
                CheckEvent(move);
            }
            if (FirstMove)
            ...
            if (move.Checkmate == false && move.Stalemate == false && move.Check == false)
            {
                if (move.CapturedPiece != Piece.NONE)
                    CapturedPieceEvent(move);
                else
                    Moved(move);
            }
            Moves.Add(move);
